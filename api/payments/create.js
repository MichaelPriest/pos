...
export async function resumeExisting(order,provider,secret){if(!order.payment_reference)return null;if(provider==='stripe'){const response=await fetch(`https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(order.payment_reference)}`,{headers:{Authorization:`Bearer ${secret||process.env.STRIPE_SECRET_KEY}`}}),data=await response.json();if(response.ok&&data.status==='open'&&data.url)return{url:data.url,reference:data.id,resumed:true};return null;}if(provider==='mercadopago'){const response=await fetch(`https://api.mercadopago.com/checkout/preferences/${encodeURIComponent(order.payment_reference)}`,{headers:{Authorization:`Bearer ${secret||process.env.MERCADOPAGO_ACCESS_TOKEN}`}}),data=await response.json();if(response.ok&&(data.init_point||data.sandbox_init_point))return{url:data.init_point||data.sandbox_init_point,reference:data.id,resumed:true};return null;}if(provider==='pagbank'){const response=await fetch(`https://api.pagseguro.com/orders/${encodeURIComponent(order.payment_reference)}`,{headers:{Authorization:`Bearer ${secret||process.env.PAGBANK_TOKEN}`}}),data=await response.json();if(response.ok&&data.qr_codes?.[0])return{qr_code:data.qr_codes[0].text,qr_image:data.qr_codes[0].links?.find(link=>link.media==='image/png')?.href,reference:data.id,resumed:true};return null;}return null;}
