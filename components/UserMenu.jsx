...
export default function UserMenu() {
  ...
  return <div className="user-menu"><button onClick={()=>setOpen(!open)}>{profile.avatar_url?<img src={profile.avatar_url} alt="Foto do perfil"/>:<span>{profile.name?.slice(0,2).toUpperCase()||'US'}</span>}<i>⌄</i></button>{open&&<div className="user-popover"><header>{profile.avatar_url?<img src={profile.avatar_url} alt=""/>:<span>{profile.name?.slice(0,2).toUpperCase()}</span>}<div><strong>{profile.name}</strong><small>{profile.email}</small><em>{employee?'Equipe · '+profile.role:'Cliente'}</em></div></header><label>📷 Alterar foto<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload}/></label><Link href={employee?'/perfil':'/minha-conta'}>👤 {employee?'Meu perfil profissional':'Minha conta'}</Link>{employee&&<Link href={profile.role==='cashier'?'/pdv':'/admin'}>⚙️ Área restrita</Link>}<button onClick={()=>{auth.signOut();location.href='/login'}}>↪ Sair da conta</button></div>}</div>;
}
