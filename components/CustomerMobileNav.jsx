...
export default function CustomerMobileNav(){
  ...
  return <nav className="customer-mobile-nav" aria-label="Atalhos da loja">{links.map(([to,icon,label])=><NavLink key={label} to={to} className={pathname===to.split('?')[0]?'active':''}><i>{icon}</i><span>{label}</span></NavLink>)}</nav>;
}
