...
export default function Store() {
  const [products,setProducts]=useState(demo),[cart,setCart]=useState([]),[query,setQuery]=useState(''),[category,setCategory]=useState('Todos'),[open,setOpen]=useState(false),[loading,setLoading]=useState(configured),[message,setMessage]=useState(''),[session,setSession]=useState(null),[settings,setSettings]=useState({store_name:'ReVeste',tagline:'Moda circular para histórias que continuam.',primary_color:'#315d4a',accent_color:'#b36f53',hero_title:'Estilo que renasce com você.',hero_subtitle:'Peças únicas, escolhidas com carinho para durar muitas histórias — inclusive a sua.'});
  ...
  return <><Head><title>{settings.store_name} | Brechó online</title><meta name="description" content={settings.tagline}/></Head><div className="store-page" style={{'--green':settings.primary_color,'--brand-accent':settings.accent_color}}>
    <header className="store-header"><Link href="/loja">{brand}</Link><nav><a href="#novidades">Novidades</a><a href="#catalogo">Comprar</a><a href="#sobre">Nossa história</a></nav><div className="store-tools"><button onClick={()=>document.querySelector('.store-search input').focus()}>⌕</button><Link href={session?'/minha-conta':'/login'}>♙</Link><button onClick={()=>setOpen(true)}>Sacola <b>{cart.length}</b></button></div></header>
    ...
    <footer className="store-footer">{brand}<p>{settings.tagline}</p><small>© 2026 {settings.store_name}. Feito com propósito.</small><Link href="/admin">Área administrativa</Link></footer>
