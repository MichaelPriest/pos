import { decrypt } from '../admin/payment-settings';
const providers = {
  async stripe(order, origin, email, secret) {
...
export default async function handler(req,res){
  ...
  try{
    ...
    const secret=await providerSecret(provider,supabase,service);const origin=process.env.NEXT_PUBLIC_SITE_URL||`https://${req.headers.host}`;const payment=await providers[provider](orders[0],origin,user.email,secret);
