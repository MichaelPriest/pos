...
test('agregação de peças fecha reduce antes das demais propriedades',()=>{assert.match(page,/items:items\.reduce\(\(sum,item\)=>sum\+Number\(item\.quantity\|\|0\),0\),categories:/);assert.doesNotMatch(page,/reduce\(\(sum,item\)=>sum\+Number\(item\.quantity\|\|0\),0,categories:/)});
